...
$renderer = $PAGE->get_renderer('format_stunning');
...
if (!empty($displaysection)) {
    ...
    user_preference_allow_ajax_update('stunning_toggle_' . $course->id, PARAM_TEXT);
    $userpreference = get_user_preferences('stunning_toggle_' . $course->id);
    $renderer->set_user_preference($userpreference);

    $defaultuserpreference = clean_param(get_config('format_stunning', 'defaultuserpreference'), PARAM_INT);
    $renderer->set_default_user_preference($defaultuserpreference);

    $PAGE->requires->js_init_call('M.format_stunning.init', array(
        $course->id,
        $userpreference,
        $course->numsections,
        clean_param(get_config('format_stunning', 'defaulttogglepersistence'), PARAM_INT),
    ...
    #morecourse ul.courses li a:hover {
       background-color: <?php
                        if ($tcsettings['togglebackgroundhovercolour'][0] != '#') {
                            echo '#';
                        }
...
$PAGE->requires->js('/course/format/stunning/format.js');
